...
# --- Futures ---
FUT_MODEL_DIR = 'crypto/futures_models'
FUT_TRADE_HISTORY_FILE = 'crypto/futures_trade_history.json'
FUT_MODEL_METADATA_FILE = 'crypto/futures_model_metadata.json'
best_futures_score = -np.inf
best_future = None
best_futures_params = None
best_futures_model = None
best_futures_features = None
best_futures_close = None
final_futures_interval = None
...
class FuturesTrader:
    def __init__(self):
        self.futures_models = {}
        self.futures_trade_history = []
        self.futures_model_metadata = {}
        self.futures_load_state()

    def futures_load_state(self):
        """Load saved models and trade history"""
        # Load trade history
        if os.path.exists(FUT_TRADE_HISTORY_FILE):
            with open(FUT_TRADE_HISTORY_FILE, 'r') as f:
                self.futures_trade_history = json.load(f)

        # Load model metadata
        if os.path.exists(FUT_MODEL_METADATA_FILE):
            with open(FUT_MODEL_METADATA_FILE, 'r') as f:
                self.futures_model_metadata = json.load(f)

        # Load models
        for threshold in THRESHOLDS:
            model_file = f"{FUT_MODEL_DIR}/futures_model_{threshold:.4f}.joblib"
            if os.path.exists(model_file):
                self.models[threshold] = {
                    'model': load(model_file),
                    'last_retrain': datetime.fromisoformat(
                        self.futures_model_metadata.get(str(threshold), {}).get('last_retrain', '2000-01-01')
                    )
                }
    ...
    def fetch_kraken_futures_movers(self):
        """Find potential trading candidates"""
        print("🔍 Scanning for futures candidates...")
        ...
        try:
            ...
            for item in data['tickers']:
                if 'pair' in item and 'change' in item:
                    change_1h = item['change'].get('1h', 0)
                    change_24h = item['change'].get('24h, 0)
                    if abs(change_1h) >= 0.025 and abs(change_24h) >= 0.05:
                        movers.append({
                            'symbol': item['pair'],
                            'change_1h_%': round(change_1h * 100, 2),
                            'change_24h_%': round(change_24h * 100, 2),
                            'mark_price': item.get('markPrice'),
                            'timestamp': datetime.utcnow()
                        })

            df = pd.DataFrame(movers)
            return df.sort_values(by='change_1h_%', ascending=False)

        except Exception as e:
            print(f"⚠️ Error scanning candidates: {str(e)}")
            return []

   # def fetch_futures_data(self, coin_id, interval={"5m": "90d", "10m": "90d", "15m": "90d"}, max_attempts=3):
            """Fetch data from ??? with retries"""
            attempts = 0
            intervals_to_try = list(interval.items())
    ...
    def calculate_futures_features(self, df):
        """Add technical indicators"""
        if df.empty:
            return None
        try:
    ...
    def train_futures_model(self, coin, df):
        """Train or update a model"""
        global best_futures_score, best_futures, best_futures_params, best_futures_model, best_futures_features, best_futures_close

        print(f"📊 Attempting to train futures model")
    ...
    def prepare_futures_training_data(self, df, threshold, window):
        """Create labeled dataset"""
        try:
          df = df.copy()
          df['future_max'] = df['close'].shift(-window).rolling(window, min_periods=1).max()
    ...
    def evaluate_futures_model_profitability(self, df, model, features, close, threshold):
        """Evaluate model profitability metrics."""
        if df.empty or model is None:
            print("❌ Empty DataFrame or model is None")
            return None
    ...
    def record_futures_trade(self, symbol, threshold, features, prediction, entry_price, entry_time):
        """Store trade details for later outcome evaluation"""
        trade_id = hashlib.md5(
            f"{symbol}{threshold}{entry_time.isoformat()}".encode()
        ).hexdigest()
    ...
    def evaluate_pending_futures_trades(self, delay_minutes):
        """Evaluate outcome of pending trades"""
        now = datetime.now()
        for trade in self.trade_history:
            if trade['status'] != 'pending':
    ...
    def simulate_futures_sell_signal(self, df, model, features, close, steps, increase_rate):
        """Simulate future price movements to predict sell signals."""
        if df.empty or model is None:
            print("❌ Empty DataFrame or model is None")
            return None
    ...
    def retrain_futures_models(self):
        """Retrain models with new trade data"""
        recent_trades = [
            t for t in self.trade_history
            if (datetime.now() - datetime.fromisoformat(t['timestamp'])).days < 7
    ...
    def run_futures_analysis(self):
        """Main analysis pipeline"""
        print("🚀 Starting futures trading algorithm")
        # 1. Scan for candidates
        candidates = self.fetch_kraken_futures_movers()
        ...
        for coin in candidates[:10]:  # Limit to top 10
            print(f"\n📊 Analyzing {coin['symbol']}")
            df = self.fetch_futures_data(coin['id'])
            if df is None:
                continue

            df = self.calculate_futures_features(df)
            if df is None:
                continue

            # 3. Train/update models for each threshold
            self.train_futures_model(coin, df)

        if best_futures_model is None:
          current_time_est = datetime.now(EST)
          print(f"⚠️ No valid models found {current_time_est.strftime('%Y-%m-%d %H:%M:%S %Z%z')}")
          return

        print(f"\n🏆 Running final analysis on {best_coin['symbol']}")
        df = self.fetch_kraken_futures_movers()(best_coin['id'])
        df = self.calculate_futures_features(df)

        close = best_futures_close
        current_price = df[close].iloc[-1]
        current_time = df.index[-1].tz_convert('US/Eastern')

        # 4. Make predictions and simulate trades
        current_features = df[best_futures_features].iloc[-1:].values
        prediction = best_futures_model.predict(current_features)[0]
        proba = best_futures_model.predict_proba(current_features)[0][1]
        ...
        if prediction == 1 and proba > 0.5:  # Confidence threshold
          ...
          message = [
              f"📈 {best_coin['symbol']} Futures Analysis Results (Threshold: {best_params[0]*100}%, Window: {best_params[1]} min)",
              f"🕒 Time: {current_time.strftime('%m-%d-%Y %I:%M %p %Z')}",
              f"💰 Price: ${current_price:.2f}",
              f"🔮 Signal: {'BUY' if prediction == 1 else 'HOLD'} ({proba:.1%} confidence)"
          ]
          ...
          # Predict sell signal
          sell_info = self.simulate_futures_sell_signal(
              df,
              best_model,
              best_features,
              best_close,
          ...
          # Record price based on threshold and window
          self.evaluate_pending_futures_trades(best_params[1])

        else:
          print("❌ No buy signal predicted")
    
        self.save_futures_state()
